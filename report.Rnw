...
Apparently only Trump is using an android device, his staff uses iOS devices. 
So it would be interesting to know who is actually typing the tweet.
We were following along with \url{http://varianceexplained.org/r/trump-tweets/} and adopted their approach for our data.
Following graph shows what time of day is tweeted from Android device and when is it tweeted from iOS device. It is visible, but not as much as in original data, that Trump tweets more in the morning while his staff is more active later in the day.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% disabled because was very slow. Strange should have been quicker to compute.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%<<label="wordcloud", results='markup', cache=TRUE>>=
