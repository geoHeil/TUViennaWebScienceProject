...
We used the degree filter in gephi to filter the graph to a minimal degreee and to remove nodes with very few connections after the Giant Component filter to make sur to include only the main vertices.
The edges are coloured with the type of connection (retweet, mention).
