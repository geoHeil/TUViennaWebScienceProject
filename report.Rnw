...
\begin{document}
\title{Web Science TU Vienna Terrorists graph analysis project}
\author{me and you}
\maketitle
\tableofcontents
...
\chapter{Replacement for collection of data}

%wie besprochen, falls Sie für das Projekt die Daten verwenden wollen, spricht nichts dagegen. 

%Wenn sich Ihre Gruppe dazu entschließt, dann aber jedenfalls:
%- die Daten und deren Kontext  genau beschreiben (Hintergrundinformation zum Setting und deskriptive Statistik vom ganzen Datensatz).
%- falls Sie ein Teilsample zur Analyse auswählen, argumentieren, warum dieses Teilsample (deskriptive Statistik auch vom Sample).
%- andere, die es zu den Daten schon gibt und die eventuell für die Analyse relevant sind, erwähnen und in die Diskussion/Interpretation einbeziehen.

%Das muss alles nicht so ausführlich sein, aber Ziel sollte sein, ein guten Überblick zu geben.

\todo{write about other kaggle solutions}
foo bar super
