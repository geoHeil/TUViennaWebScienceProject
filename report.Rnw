...
We will analyze wordclouds for different communities, i.e. connected components. For the largest 3 the wordcounts will be depicted below.
\todo{make the code work / build a foor loop}
%<<label="wordcloudCommunities", results='markup', cache=FALSE, warning=FALSE>>=
%@
...
\todo{text is no longer valid, still same problem}
...
\todo{answer remaining questions}
What can you tell about the communities identified? What do you learn by looking at the different networks?
...
report \todo{fix remaining todos and upload}
presentation \todo{todo}
\end{document}
